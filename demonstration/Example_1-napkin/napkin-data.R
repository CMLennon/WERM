...
Discretizer = function(valcal, myvec){
  myDivider = seq(0,1,len=valcal+1)
  mydiscvec = mapply(function(val){
    for (idx in c(1:length(myDivider))){
      if (val <= myDivider[idx+1]){
        return(idx-1)
      }
    }
  },myvec)
...
dataGen = function(seednum,N,Nintv,D,C){
  ...
  fW = function(N,U1,U2){
    ...
    for (idx in 1:D){
      myval = inv.logit(c1[idx]*U1+c2[idx]*U2 -2*(U1*U2+2) + Uw+1)
      # myval = inv.logit(-0.2*exp(c1[idx]*U1+c2[idx]*U2 + Uw))
      W[,idx] = Discretizer(Wcal,myval)
      # W[,idx] = rbinom(N,size=1,prob=inv.logit(-0.2*exp(c1[idx]*U1+c2[idx]*U2 + Uw)))
    }
    W = data.frame(W)
  ...
  fZ = function(N,W){
    ...
    Z = inv.logit(-1*Zval + Uz-1)
    Z = Discretizer(Zcal,Z)
    return(Z)
  }
  fX = function(N,U1,Z){
    Ux = rnorm(N,1,2)
    Xvec = inv.logit(1*(2*Z-1) - 5*U1 + Ux   )
    X = Discretizer(Xcal,Xvec)
    
    # X = rbinom(N,size=1,inv.logit(log(abs(1*(U1*Z))+1)* 2*(2*Z-1)*Ux - 4*Ux*exp(U1-2) -3))
    # X = rbinom(N,size=1,inv.logit(1*(2*Z-1) - 5*U1 + Ux   ))
    # X = xor(rbinom(N,size=1,inv.logit(-1*U1+Ux)),Z)*1
    return(X)
  }
  fY = function(N,U2,X){
    Uy = rnorm(N,-2,1)
    ind.X = 2*X - 1
    Ycal = 2 
    Yval = inv.logit(-1*ind.X* U2*log(abs(U2*ind.X)+1) + 0.1*U2-Uy)
    Y = Discretizer(Ycal,Yval)
    
    # Y =1*(2*U2 + ind.X- Uy )
    # Y = inv.logit(Y)
    
    # Y = rbinom(N,size=1,inv.logit(3*exp(U2) - 2*ind.X + Uy))
    # Y = rbinom(N,size=1,inv.logit(-1*ind.X* U2*log(abs(U2*ind.X)+1) + 0.1*U2-Uy))
    # Y = xor(rbinom(N,size=1,inv.logit(-1*U2+Uy)),X)*1
  ...
  X.intv = c()
  for (idx in c(1:(Xcal-1))){
    X.intv = c(X.intv,rep((idx-1),Nintv/Xcal))
  }
  X.intv = c(X.intv,rep(idx,(Nintv-length(X.intv))))
